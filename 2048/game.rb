...
class Game
	...
	def fill_random_empty_square
		...
		init_vals = [1, 2]
		@squares[empty_squares.sample].set(init_vals.sample)
	end
...
#rewrite this section here
  			if arr[i] != 0
    			for j in i+1..arr.length-1 do
      				if arr[j] != arr[i]-1 and arr[j] != arr[i]+1 and arr[j] != 0
								break
      				elsif arr[j] == arr[i]-1 || arr[j] == arr[i] + 1
        				arr[i] = [arr[i],arr[j]].max + 1
        				## check for win state
        				if arr[i] == 11
        					@win = true
        				end
